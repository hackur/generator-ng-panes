...
var PANES_DIR = '.panes';
var PREFERENCE_FILENAME = path.join(PANES_DIR , 'ng-panes-preference.json');
var PACKAGE_NAME = 'generator-ng-panes';
var NG_PANES_CONFIG_FILE = '.ng-panes-config.json';
var PANES_CONFIG_FILE = '.panes-config.json';
// define the main
...
var searchFile = function()
{
    ...
    var search = path.join(home , PREFERENCE_FILENAME);
...
var runVersionCheck = function(localVersion , checkDate , lang)
{
    ...
    viewPackage(PACKAGE_NAME).then(function(result) {
        if (result && result!==localVersion) {
            var msg = chalk.white('---------------------------------------------------\n');
                 msg += chalk.white( (lang==='cn') ? PACKAGE_NAME + '现时安装的版本为' : 'Your `' + PACKAGE_NAME + '` installed version is: ');
                 ...
                 msg += chalk.yellow('npm install ' + PACKAGE_NAME + ' -g \n');
                 msg += chalk.white('---------------------------------------------------');
            console.log(msg);
        }
        defer.resolve(1);
 ...
 */
var checkPanesConfig = function()
{
    try {
    	var panesConfigFile = path.join(process.cwd() , PANES_CONFIG_FILE);
        if (existsFile(panesConfigFile)) {
            return require(panesConfigFile); // return the json to set some default options
        }
        return false;
    } catch(e) {
        console.log(e);
...
var upgradeProjectFile = function(json , localVersion , today)
{
    if (_.isUndefined(json.app)) {
        ...
        fs.writeFile(path.join(home , PREFERENCE_FILENAME) , JSON.stringify(data , null , 2) , function(err)
        {
            if (err) {
...
{
    var home = getUserHome();
    var _path = path.join(home , PANES_DIR);

    if (!existsFile(_path)) {
        fs.mkdirSync(_path);
    }
    return fs.writeFile(path.join(home , PREFERENCE_FILENAME) , JSON.stringify(data , null , 2) , callback);
};
...
{
    try {
        var config = require(path.join(getUserHome() , NG_PANES_CONFIG_FILE));
        if (key && config[key]) {
            return config[key];
        }
        return config;
