 ...
 * run a remote check
 */
var runVersionCheck = function(localVersion , checkDate)
{
    var defer = q.defer();
    var toDay = getDate();
    if (toDay !== checkDate) {
        ...
        viewPackage(packageName).then(function(result) {
            if (result && result!==localVersion) {
                console.log(chalk.white('Your installed version is: ') + chalk.yellow(localVersion) + chalk.white('. The latest version is ') + chalk.yellow(result) + chalk.white('. To upgrade please run ') + chalk.yellow('npm install ' + packageName + ' -g'));
            }
            defer.resolve(1);
        });
    }
    else {
        // console.log(chalk.white('Your installed version is: ') + chalk.yellow(localVersion));
        defer.resolve(1);
    }
    return defer.promise;
};
...
var initProjectFile = function(localVersion , today)
{
    ...
    if (!existsFile(search)) {
        ...
        var fileToInit = path.join(home , filename);

        fs.mkdirSync(path.join(home , dir));
        fs.writeFile(fileToInit , JSON.stringify(data , null , 2) , function(err)
        {
            if (err) {
...
Preference.init = function()
{
    ...
    async.waterfall([
        ...
        function(result , callback) {
            runVersionCheck(result.app.version , result.app.date).then(function(done)
            {
 ...
 */
Preference.findProjects = function()
{
    var json = Preference.find();
    if (json) {
        var sort = {};
