    ...
    if (toDay !== checkDate) {
        ...
        viewPackage(packageName).then(function(result) {
            if (result && result!==localVersion) {
                var msg = chalk.white('Your installed version is: ')
                       += chalk.yellow(localVersion)
                       += chalk.white('. The latest version is ')
                       += chalk.yellow(result)
                       += chalk.white('. To upgrade please run ')
                       += chalk.yellow('npm install ' + packageName + ' -g');
                console.log(msg);
            }
            defer.resolve(1);
        });
...
Preference.init = function(lang)
{
    ...
    async.waterfall([
        ...
        function(result , callback) {
            runVersionCheck(result.app.version , result.app.date , lang).then(function(done)
            {
